...
export default function Popup()  {
	...
	const destinationItems = destinations.map((destination, index) => {
		return (
			<>
			<TextField id="filled-basic" label={`Destination ${index + 1}:`} variant="filled" value={destination} InputProps={{
				startAdornment: (
		...
		{index === destinations.length - 1 && <Button variant="contained" disabled={destination === ""} onClick={() => handleAddInput()}>Add destination</Button>}
			</>
		)
	})
	...
	return (
		...
		>
			<Box
				...
				style={style}
			>
				<TextField id="filled-basic" label="Title" variant="filled" InputProps={{
					startAdornment: (
						<InputAdornment position="start">
							<TitleIcon />
						</InputAdornment>
				...
				<TextField id="filled-basic" label="Budget" variant="filled" InputProps={{
					startAdornment: (
						<InputAdornment position="start">
							<MonetizationOnIcon />
						</InputAdornment>
